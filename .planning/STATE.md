...
- Phase 10 COMPLETE (2026-07-15, inline 1/1): реализовано как **overlay finalize pass** — ВТОРОЙ ffmpeg-проход над готовым клипом (`render.build_overlay_pass_command` + пост-блок в `render_clip`), НЕ вплетено в 3 основных билдера (jumpcut/profanity/compilation-графы остались байт-идентичными при выкл). Fail-open. Outro: самоанимирующийся `gradients` фон (`render.OUTRO_PATTERNS`, 5 пресетов, ротация `queue_index % pattern_count`) + drawtext ник/CTA с fade-in + глиф overlay + matched-silence аудио, конкат base+card (база ресемплится в `outro_fps`). Popups: drawbox+overlay+drawtext на одном slide-in x-выражении (`_popup_slide_x`), twitch с иконкой / kick текст-заглушка, `plan_popup_times` разносит по клипу. Кэп: `candidates.compute_moment_budget` (глобальный по всей записи) + `candidates.select_top_moments` по `score`, оркестрируется в SKILL.md шаг 4 только в авто-режиме. Конфиг-секции `social_overlay`/`outro_card`/`top_moments`. Грабли: `-map [0:a]` для raw-инпута невалиден в filter_complex — маппить `0:a` без скобок. Реальный ffmpeg 8.1.2 (gradients/drawbox/anullsrc есть). Сюита 700 passed / 5 skipped.
