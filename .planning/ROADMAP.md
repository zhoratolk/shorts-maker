...
- [x] 05-01-PLAN.md — Candidate dataclass gains tag/sub_threshold/group_id/unmatched fields + CANDIDATES.md compilation/unmatched sections [COMP-01] (wave 1)
- [x] 05-02-PLAN.md — ClipConfig.compilation_max_seconds + scripts/compilation.py mechanical group validation/PLAN.json entry builder [COMP-01, COMP-02, COMP-03] (wave 1)
- [x] 05-03-PLAN.md — render.py::build_compilation_command multi-input stitch + render_clip compilation dispatch [COMP-02] (wave 1)
...
- [x] 06-03-PLAN.md — TikTok core: queue lifecycle, OAuth (cache/refresh + one-time consent), Content Posting API HTTP layer, D-05 SELF_ONLY-detecting orchestration + reconciliation [PUB-06] (wave 2)
- [x] 06-04-PLAN.md — Instagram core: queue lifecycle, OAuth, resumable-upload HTTP layer, attempt-then-fail-closed orchestration (InstagramAccessError) + reconciliation [PUB-07] (wave 2)
...
| 7. Profanity Auto-Bleep | 0/5 | Planned | - |
...
- [ ] 07-01-PLAN.md — Detection module: scripts/profanity.py (load_wordlist/normalize_word/compile_patterns/find_profane_spans + CLI) + committed data/profanity_wordlist.yaml + tests [AUDIO-01] (wave 1)
...
- [ ] 07-04-PLAN.md — Real-ffmpeg loudness-delta + faster-whisper self-transcription integration tests + SKILL.md orchestration (profanity_spans in PLAN.json, step-5/5b bullets, step-6 flags, recall-gap note) [AUDIO-01, AUDIO-02, AUDIO-03] (wave 2)
