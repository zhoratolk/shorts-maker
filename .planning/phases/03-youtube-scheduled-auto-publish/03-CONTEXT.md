...
- **D-08:** Upload uses a **separate `upload_token.json`** (own OAuth consent), distinct from `youtube_analytics.py`'s existing read-only `token.json` (scopes: `youtube.readonly` + `yt-analytics.readonly`). Reuses the same `load_credentials()`-style pattern (parameterized by scopes/token path already exists in `scripts/youtube_analytics.py`) rather than inventing a new OAuth flow.
- **D-09 (rationale, amended 2026-07-08):** Originally scoped `upload_token.json` to the narrower `https://www.googleapis.com/auth/youtube.upload` for smaller blast radius if the token leaks — relevant given this project's prior real leaked-data incident (see STATE.md Blockers/Concerns). A live kill-path test on 2026-07-08 proved that scope insufficient: `videos.update` (used by `cancel_scheduled_release`/`kill_item`, the pause/kill safety mechanism required by the project's "Необратимость авто-паблиша" constraint) returned `403 insufficientPermissions` under `youtube.upload` alone. The publish went live briefly before the operator manually deleted the video via YouTube Studio. Scope was widened to the full `https://www.googleapis.com/auth/youtube` so the kill/pause path actually works — a real, larger blast radius traded for a working safety mechanism, since a safety mechanism that cannot execute is not a safety mechanism. `client_secret.json`/`token.json`/`upload_token.json` all stay gitignored, same discipline as before.
