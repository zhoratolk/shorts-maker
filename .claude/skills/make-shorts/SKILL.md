...
- The poster is written next to the clip. In the publish step, pass its path as `thumbnail_path` to `enqueue` (along with the candidate's `moment_tag` and the clip's `source_duration`) — `config.thumbnail.upload` then controls whether `publish_queue` sets it as the video's custom thumbnail after upload (fail-open). See [docs/publish-queue.md](docs/publish-queue.md).
