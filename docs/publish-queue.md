...
Превью-кадр для Shorts через API тоже не выбрать (thumbnails.set на ленту
Shorts не влияет) — если для клипа критичен конкретный кадр-обложка, это
делается только при ручной загрузке с телефона.
